...
class LivrosRepository {
    ...
    async findAll() {
        const rows = await db.all(`
            SELECT id, titulo, autor, categoria, ano, editora, numeroPaginas
            FROM livros
            ORDER BY id ASC
        `);
    ...
    async create(data) {
        ...
        const res = await db.run(`
            INSERT INTO livros (titulo, autor, categoria, ano, editora, numeroPaginas)
            VALUES (?, ?, ?, ?, ?, ?)
    ...
    async update(id, dados) {
        ...
        await db.run(`
            UPDATE livros
            SET titulo = ?, autor = ?, categoria = ?, ano = ?, editora = ?, numeroPaginas = ?
            WHERE id = ?
